...
| 结论 | 证据 |
|---|---|
| 秩 = 带宽 × 延迟占据测度 | 合成场斜率 0.985 / R²=0.998（n=840）；FDTD 0.923/0.944；maze 0.938/0.954 |
| 秩由**带宽**决定，与中心频率无关 | rank~带宽 R²=0.96–0.97，rank~中心频率 R²=0.001–0.005 |
| 收益 = 占据测度之比，可预先算出 | 区间相图上从 6.00× 单调塌缩到 1.07×，混响区间预测误差 <5% |
| 表征收益 → 任务收益 | log-log 斜率 1.49，R²=0.988（36 个 case，2% 传感器） |
| 载波精度判据 `δτ < 1/B` | 粗糙误差在 1/B 处收益归零；平滑标定误差在 2.5/B 仍有 2.7× |
| 判据可直接选载波，代价为零 | 93% 选中最优，拿到 95% 的 oracle 增益 |
| 多载波：等预算下 1.41× 优于单载波 | 优势随体散射从 1.41× 衰减到 1.06×，与预测形状一致 |
| 载波可完全从数据估计，无需几何 | 拿到 oracle 的 90–96%，波前误差 0.02–0.08 ×(1/B) |
| 公开数据：staircase 等预算压缩 9.8–11.5× | The Well Helmholtz staircase，可部署 eikonal 载波 |
| Fourier features / SIREN 不能替代载波 | 原始场上测试 NRMSE 0.82–1.09，训练 NRMSE 0.000–0.015 |
| maze 上无收益——**这是被预测出来的** | `Λ_rel≈Λ_abs`，预测 1.06、实测 1.02 |
